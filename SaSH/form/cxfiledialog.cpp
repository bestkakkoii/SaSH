...
cx::file_dialog::file_dialog(long long mode, HWND parent = nullptr)
	: parent_(parent)
	, directory_("")
	, caption_("")
...
HRESULT cx::file_dialog::setNameFilters()
{
	...
	long long sepPos = 0;
	long long size = nameFilters_.size();
	QString name;
	QString spec;
	QString temp;
...
HRESULT cx::file_dialog::handleFilesOrDirectoriesSelection()
{
	...
	do
	{
		...
		for (DWORD i = 0; i < itemCount; ++i)
		{
			...
			{
				selectedFiles_.append(QString::fromWCharArray(pszFilePath));
