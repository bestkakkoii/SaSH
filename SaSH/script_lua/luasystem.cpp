...
#pragma region zh_TW
	...
	QString key = QString::fromUtf8(enumStr.c_str());
	if (!hash.contains(key))
		return FALSE;

	util::UserSetting setting = hash.value(key);
	...
	case util::kBattleNoEscapeWhileLockPetEnable:
	{
		...
		if (p1.is<bool>())
			...
			value = p1.as<qint64>() > 0;
		else
			return FALSE;

		Injector& injector = Injector::getInstance();

		injector.setEnableHash(setting, value);
