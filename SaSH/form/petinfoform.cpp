...
PetInfoForm::PetInfoForm(__int64 index, QWidget* parent)
	: QWidget(parent)
	, Indexer(index)
{
	ui.setupUi(this);
...
void PetInfoForm::on_comboBox_currentIndexChanged(int index)
{
	...
	QVector<__int64> v = ui.comboBox->currentData().value<QVector<__int64>>();
	cur_level_ = v[0];
	cur_maxHp_ = v[1];
	cur_atk_ = v[2];
	cur_def_ = v[3];
	...
	//__int64 count = 0;
	QVector<__int64> basev(MAX_PET, 0);
	if (i.hasMatch())
	{
	...
	{
		__int64 currentIndex = ui.comboBox->currentIndex();
...
void PetInfoForm::on_comboBox_clicked()
{
	...
	__int64 current = ui.comboBox->currentIndex();
	ui.comboBox->clear();
...
constexpr inline double calcRate(__int64 diff, __int64 diff_level)
{
	return diff / static_cast<double>(diff_level);
}

constexpr inline double calcExpectValue(double base, double growthRate, __int64 targetLevel)
{
...
void PetInfoForm::on_pushButton_calc_clicked()
{
	...
	__int64 diff_level = cur_level_ - base_level_;
	__int64 diff_maxHp = cur_maxHp_ - base_maxHp_;
	__int64 diff_atk = cur_atk_ - base_atk_;
	__int64 diff_def = cur_def_ - base_def_;
	__int64 diff_agi = cur_agi_ - base_agi_;
	...
	__int64 expect_level = ui.spinBox_expect_level->value();
