...
static void whilestat(LexState* ls, int line)
{
	...
	condexit = cond(ls);
	enterblock(fs, &bl, 1);
	//checknext(ls, TK_DO); fix 11/14/2022
	if (ls->t.token == TK_DO)
	{
		luaX_next(ls);
	}
...
static void test_then_block(LexState* ls, int* escapelist)
{
	...
	expr(ls, &v);  /* read condition */
	//checknext(ls, TK_THEN);//fix by 11/14/2022
	if (ls->t.token == TK_THEN)//Check that next token is 'TK_THEN' and skip it. //fix by 11/14/2022
	{
		luaX_next(ls);
	}
