...
bool Autil::util_deint(long long sliceno, long long* value, long long* sum)
{
	QMutexLocker locker(&msgMutex_);
...
bool Autil::util_destring(long long sliceno, char* value, long long* sum)
{
	QMutexLocker locker(&msgMutex_);
	if (!msgSlice_.contains(sliceno))
		return false;
	...
	*sum = static_cast<long long>(strlen(value));
	return true;
}
