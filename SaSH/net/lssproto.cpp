...
{
	autil_->util_Send(sa::LSSPROTO_CHARLOGOUT_SEND, Flg);
}

//往指定方向查看觸發對話的效果
void Lssproto::lssproto_L_send(long long dir)
{
	autil_->util_Send(sa::LSSPROTO_L_SEND, dir);
}

//遇敵
void Lssproto::lssproto_EN_send(const QPoint& pos)
{
	autil_->util_Send(sa::LSSPROTO_EN_SEND, pos.x(), pos.y());
}

//退出觀戰
void Lssproto::lssproto_BU_send(long long dummy)
{
	autil_->util_Send(sa::LSSPROTO_BU_SEND, dummy);
}

//開關封包
void Lssproto::lssproto_FS_send(long long flg)
{
	autil_->util_Send(sa::LSSPROTO_FS_SEND, flg);
}
...
{
	autil_->util_Send(sa::LSSPROTO_WN_SEND, pos.x(), pos.y(), dialogid, unitid, select, data);
}

//設置寵物狀態封包  0:休息 1:戰鬥或等待 4:郵件
void Lssproto::lssproto_PETST_send(long long nPet, long long sPet)
{
	autil_->util_Send(sa::LSSPROTO_PETST_SEND, nPet, sPet);
...
{
	autil_->util_Send(sa::LSSPROTO_KS_SEND, petarray);
}
...
{
	autil_->util_Send(sa::LSSPROTO_SHOPOK_SEND, n);
}
...
{
	autil_->util_Send(sa::LSSPROTO_HL_SEND, flg);
}
...
void Lssproto::lssproto_B_send(const QString& command)
{
	...
	autil_->util_Send(sa::LSSPROTO_B_SEND, const_cast<char*>(cmd.c_str()));
}

void Lssproto::lssproto_MA_send(const QPoint& pos, long long nMind)
{
	autil_->util_Send(sa::LSSPROTO_MA_SEND, pos.x(), pos.y(), nMind);
}

void Lssproto::lssproto_S2_send(char* data)
{
	autil_->util_Send(sa::LSSPROTO_S2_SEND, data);
}
...
void Lssproto::lssproto_CreateNewChar_send(
	...
	long long hometown)
{
	autil_->util_Send(sa::LSSPROTO_CREATENEWCHAR_SEND, dataplacenum, charname, imgno, faceimgno, vital, str, tgh, dex, earth, water, fire, wind, hometown);
}

//刪除人物
void Lssproto::lssproto_CharDelete_send(char* charname, char* securityCode)
{
	autil_->util_Send(sa::LSSPROTO_CHARDELETE_SEND, charname, securityCode);
}
...
{
	autil_->util_Send(sa::LSSPROTO_CHARLOGIN_SEND, charname);
}